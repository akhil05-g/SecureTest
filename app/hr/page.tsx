...
import { LiveAlertFeed } from '@/src/components/dashboard/LiveAlertFeed';
import { IncidentTimeline } from '@/src/components/dashboard/IncidentTimeline';
import { EvidenceInspector } from '@/src/components/dashboard/EvidenceInspector';
import { AuditDecisionPanel } from '@/src/components/dashboard/AuditDecisionPanel';
import { AuditExportModal } from '@/src/components/dashboard/AuditExportModal';
import { IntegrityAnalytics } from '@/src/components/dashboard/IntegrityAnalytics';
import { QuickReviewBar } from '@/src/components/dashboard/QuickReviewBar';
import { useInvestigationShortcuts } from '@/src/hooks/useInvestigationShortcuts';
import { IntegrityEvent, getStatusBadgeColor } from '@/src/types';
...
export default function HRCommandCenter() {
  ...
  const handleConfirmFlagAction = () => {
    if (!selectedCandidate) return;
    recordReviewerAction(selectedCandidate.id, 'CONFIRM_FLAG', 'Disqualification confirmed via quick hotkey.', 'Sarah Jenkins');
  };

  const handleDismissFlagAction = () => {
    if (!selectedCandidate) return;
    recordReviewerAction(selectedCandidate.id, 'DISMISS_FLAG', 'Dismissed as false positive via quick hotkey.', 'Sarah Jenkins');
  };

  const handleEscalateAction = () => {
    if (!selectedCandidate) return;
    recordReviewerAction(selectedCandidate.id, 'ESCALATE', 'Escalated to committee via quick hotkey.', 'Sarah Jenkins');
  };
